/* ============================================================
   ...
   IndexedDB handle storage. Keep this block before modules that consume these definitions.
   ============================================================ */
...
/* ============================================================
   UNITS — ИГРОВЫЕ МЕТРЫ
   Все данные (JSON зданий) хранятся в МЕТРАХ (суффикс _m в имени поля).
   ...
   загрузка, чтение комнат, поля ввода) значения переводятся в метры.
...
// Масштаб фона: в JSON хранится реальная ширина в метрах (widthKey), масштаб считаем от размера картинки.
function bgScaleFromJSON(l,nativeW,widthKey){
  const wm=l&&l[widthKey||'widthM'];
...
// JSON комнаты (Room Editor, новый или старый) → тот же объект с числами в px
function roomFromJSON(d){
  ...
  return {...d, width, height,
    backgroundLayers:(d.backgroundLayers||[]).map(l=>({...l, x:lenPx(l,'xM','x',width/2), y:lenPx(l,'yM','y',height/2)})),
    instances:(d.instances||[]).map(i=>({...i, x:lenPx(i,'xM','x',0), y:lenPx(i,'yM','y',0)}))
  };
}
...
function idbOpen(){
  return new Promise((resolve,reject)=>{
    const req=indexedDB.open('building_editor_fs',1);
...
async function tryRestoreProjectFolder(){
  try{
    ...
    if(perm==='granted'){ await scanRoomCatalog(); await scanExistingBuildings(); }
  }catch(e){ console.warn('Не удалось восстановить папку проекта:',e); }
}
async function regrantProjectFolder(){
  if(!projectDirHandle)return;
  try{ const perm=await projectDirHandle.requestPermission({mode:'readwrite'}); if(perm==='granted'){ updateFolderStatus(); await scanRoomCatalog(); await scanExistingBuildings(); } }
...
document.getElementById('btnRefreshRooms').onclick=()=>scanRoomCatalog();
