/* ============================================================
   ...
   который читает и пишет сам Object Plan — data/object_plan.json (поля custom/customGroups) —
   поэтому что бы ни добавили здесь, при следующем «📂 Из проекта» в Object Plan это появится там.
   ============================================================ */

let planCustomItems=[], planCustomGroups=[];
...
async function savePlanCustomToProject(){
  ...
  try{
    ...
    const data=Object.assign({},base,{schema_version:1,saved_at:new Date().toISOString(),custom:planCustomItems,customGroups:planCustomGroups});
...
async function addOrUpdatePlanItem({groupId,newGroupName,name,categoryId,variationRu}){
  ...
  if(vr){
    const en=sanitizeSlug(vr);
    const exists=item.v.some(v=>variationEn(v)===en||normName(variationRu(v))===normName(vr));
    if(!exists){ item.v.push(vr); created.variation=true; }
  }
