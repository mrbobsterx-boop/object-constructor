...
| Поле | Единица |
|---|---|
| `behavior.real_width_cm`, `real_height_cm` | см (целое число) — игровой размер объекта; задаётся только в ОС и не зависит от пикселей картинки. При включённой кнопке «🔗 Сохранить пропорции» (по умолчанию включена) вторая сторона подставляется автоматически по пропорциям картинки |
