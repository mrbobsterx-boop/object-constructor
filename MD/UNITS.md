...
| Поле | Единица |
|---|---|
| `behavior.real_width_cm`, `real_height_cm` | см, **кратно 10** (текущее поведение ОС — округляет при сборке JSON) — игровой размер объекта; задаётся только в ОС и не зависит от пикселей картинки. Произвольные целые см и автоподбор по пропорциям картинки описаны в дизайне ОС, но пока не реализованы — см. «Что сейчас требует внимания» в `00-README-Overview.md` |
